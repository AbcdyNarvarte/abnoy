...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        """)
        # Add status_quo column to products if it doesn't exist
        c.execute("PRAGMA table_info(products)")
        columns = [col[1] for col in c.fetchall()]
        if 'status_quo' not in columns:
            c.execute("ALTER TABLE products ADD COLUMN status_quo TEXT DEFAULT 'Pending'")
        ...
        """)
        # Add status_quo column to orders if it doesn't exist
        c.execute("PRAGMA table_info(orders)")
        order_columns = [col[1] for col in c.fetchall()]
        if 'status_quo' not in order_columns:
            c.execute("ALTER TABLE orders ADD COLUMN status_quo TEXT DEFAULT 'Pending'")
    ...
    def create_product(self, product_name, materials_list):
        ...
        c.execute("""
            INSERT INTO products (product_id, product_name, materials, created_date, status_quo)
            VALUES (?, ?, ?, ?, ?)
        """, (product_id, product_name, materials_str, timestamp, 'Pending'))
    ...
    def get_all_products(self):
        ...
        conn = self.get_connection()
        c = conn.cursor()
        c.execute("SELECT product_id, product_name, materials, created_date, status_quo FROM products ORDER BY created_date DESC")
        products = c.fetchall()
        conn.close()
        return products
    ...
    def create_order(self, order_name, product_id, client_id, quantity, deadline):
        ...
        c.execute("""
            INSERT INTO orders (order_id, order_name, product_id, order_date, order_dl, 
                              order_quantity, client_id, status_quo)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (order_id, order_name, product_id,  timestamp, deadline, 
              quantity, client_id, 'Pending'))
        conn.commit()
        conn.close()
        return order_id
    ...
    def get_all_orders(self):
        ...
        c.execute("""
            SELECT o.order_id, o.order_name, p.product_name, c.client_name, 
                   o.order_quantity, o.order_dl, o.order_date, o.product_id, o.client_id, o.status_quo
            FROM orders o
    ...
    def get_order_by_id(self, order_id):
        ...
        c.execute("""
            SELECT o.order_id, o.order_name, o.product_id, o.client_id, 
                   o.order_quantity, o.order_dl, o.order_date, o.status_quo
            FROM orders o
            WHERE o.order_id = ?
        """, (order_id,))
        order = c.fetchone()
