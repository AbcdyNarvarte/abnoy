...
class ProductPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Add a search button
        self.search_btn = CTkButton(self, text="Search", fg_color="#5dade2", width=80, command=self.search_orders)
        self.search_btn.pack(side="left", anchor="n", padx=5)
    ...
    def load_orders(self, orders):
        """Call this method to load orders into the treeview and store them for searching."""
        self.order_tree.delete(*self.order_tree.get_children())
        self.all_orders = orders  # Store all orders
        for order in orders:
    ...
    def search_orders(self):
        ...
        if not query:
            ...
            for order in self.all_orders:
                if any(query in str(value).lower() for value in order):
